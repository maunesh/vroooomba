...
public class ShortGrid {
	...
	public boolean needsAttention(int x, int z, int n, int facing) {
		...
		switch(facing) {
			case RoombaHelper.FACING_EAST:	// SOUTH
				if(getStatus(x + 1, z) == 0 || getStatus(x - 1, z) == 0 || getStatus(x, z + 1) == 0) {					
					return true;
				}
				break;
			case RoombaHelper.FACING_NORTH:	// NORTH
				if(getStatus(x - 1, z) == 0 || getStatus(x, z - 1) == 0 || getStatus(x + 1, z) == 0) {
					return true;
				}
				break;
			case RoombaHelper.FACING_SOUTH:	// EAST
				if(getStatus(x, z - 1) == 0 || getStatus(x, z + 1) == 0 || getStatus(x + 1, z) == 0) {
					return true;
				}
				break;
			case RoombaHelper.FACING_WEST:	// NORTH again (do we want WEST here)?
				if(getStatus(x - 1, z) == 0 || getStatus(x + 1, z) == 0 || getStatus(x, z - 1) == 0) {
					return true;
				}
			default:
